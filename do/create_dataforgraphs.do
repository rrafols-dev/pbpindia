...
*br area_r area_u area_t if logarea==. // administrative limitations, rural areas not recorded for some districts
tab industry, m
tab did
...
label var DID_1 "Agriculture, hunting, forestry, and fishing"
label var DID_2 "Mining and quarrying"
label var DID_3 "Labor-intensive Manufacturing"
label var DID_4 "Capital-intensive Manufacturing"
label var DID_5 "Electricity, Gas and Water"
label var DID_6 "Construction"
label var DID_7 "Wholesale trade and retail trade and restaurants and hotels"
label var DID_8 "Transport, storage, and communication"
label var DID_9 "Financing, insurance, real estate and business services"
label var DID_10 "Community, social and personal services"
label var DID_11 "Excluded manufacturing"
...
gen tot_emp_lm=log(tot_emp) if did3==1
gen tot_emp_cm=log(tot_emp) if did4==1
gen tot_emp_oi=log(tot_emp) if did3!=1 & did4!=1
replace tot_emp_lm=0 if tot_emp_lm==. & did3==1
replace tot_emp_cm=0 if tot_emp_cm==. & did4==1
replace tot_emp_oi=0 if tot_emp_oi==. & (did3!=1 & did4!=1)

gen num_firms_lm=log(num_firms) if did3==1
gen num_firms_cm=log(num_firms) if did4==1
gen num_firms_oi=log(num_firms) if did3!=1 & did4!=1 
replace num_firms_lm=0 if num_firms_lm==. & did3==1
replace num_firms_cm=0 if num_firms_cm==. & did4==1
replace num_firms_oi=0 if num_firms_oi==. & (did3!=1 & did4!=1)

drop if backwarddist==1 & weightedcountindex>500
label var work_partrate_t "Worker participation rate"
label var literacy_rate_t "Literacy rate"
label var sex_ratio_t "Number of females per 1000 males"
label var area_t "District area"
label var t_popln_t "Population"
label var res_house_t "Residential households"
label var t_worker_t "Main workers"
label var t_marginal_t  "Marginal workers"
label var t_agfc_t "Primary workers"
label var t_manuf_t "Manufacturing workers"
label var t_indcat7_t "Trade and commerce workers"
label var tot_emp "Total Employment"
label var tot_emp_lm "Labor-intensive manufacturing"
label var tot_emp_cm "Capital-intensive manufacturing"
label var num_firms "Total Firms"
label var num_firms_lm "Labor-intensive manufacturing"
label var num_firms_cm "Capital-intensive manufacturing"


*PLOTS USING DISTRICT-INDUSTRY
set more off
foreach y in "t" {
	foreach var of varlist t_popln_`y' res_house_`y'  t_worker_`y' t_marginal_`y'  t_agfc_`y' t_manuf_`y' t_indcat7_`y' area_`y'{
		replace `var'=log(`var')
	}
}
drop w1 w2 w3

label var tot_emp "Employment (log)"
label var tot_emp_cm "Employment (log)" 
label var tot_emp_lm "Employment (log)"
label var num_firms "Number of Firms (log)" 
label var num_firms_cm "Number of Firms (log)"
label var num_firms_lm "Number of Firms (log)"
save "$outreg\dataforgraphs.dta", replace

set more off
import excel using "$dta\Matrix Construction2.xls", clear sheet("STEP6") firstrow cellrange(A3:Q72)
replace NIC="888" if NIC=="E"
rename MaxFG MaxInput
rename MaxIJ MaxOutput
destring NIC, gen(industry)
keep industry MaxOutput MaxInput IOlinkage AboveMedian AveInput AveOutput AveIO AboveMedian_Ave
rename AboveMedian_Ave AveAboveMedian
rename IOlinkage MaxIO 
rename AboveMedian MaxAboveMedian
tempfile s
save `s'

set more off
import excel using "$dta\Matrix Construction3.xls", clear sheet("STEP6") firstrow cellrange(A3:Y72)
replace NIC="888" if NIC=="E"
destring NIC, gen(industry)
keep industry MaxIO_Light MaxIO_Heavy AveIO_Light AveIO_Heavy MaxInputHeavy MaxOutputLight MaxInputLight MaxOutputHeavy AveInput_Light AveOutput_Light AveInput_Heavy AveOutput_Heavy
merge 1:1 industry using `s'
assert _merge==3
drop _merge
tempfile s1
save `s1'
...
use "$dta\ec1998_dist_ind98_1.dta",clear 
...
foreach name in "Max" "Ave"{
	gen `name'bdIO=bdv2*`name'IO
	gen `name'bdIO_Light=bdv2*`name'IO_Light
	gen `name'bdIO_Heavy=bdv2*`name'IO_Heavy
}

label var bdv2 "Effects on the untreated industries in the treated districts"
label var MaxbdIO_Light "w/linkage to light(max): Untreated industries in treated districts"
label var AvebdIO_Light  "w/linkage to light(ave):Untreated industries in treated districts"
label var MaxbdIO_Heavy "w/linkage to heavy(max): Untreated industries in treated districts"
label var AvebdIO_Heavy  "w/linkage to heavy(ave):Untreated industries in treated districts"
label var MaxbdIO "w/linkage(max): Untreated industries in treated districts"
label var AvebdIO "w/linkage(ave):Untreated industries in treated districts"
...
** NB sample(taking out endogeneity) **
set more off
gen nbsample3=1 if groupcat=="250 and below" // 38 districts
gen nbsample4=1 if groupcat=="251 to 350" // 44 districts
gen nbsample5=1 if groupcat=="351 to 500" // 38 districts
gen nbsample6=1 if groupcat=="501 to 650" // 39 districts
gen nbsample7=1 if groupcat=="651 to 850" // 41 districts
gen nbsample8=1 if groupcat=="851 and above" // 157 districts


gen wTNB1=1 if NIL_NB3==0
replace wTNB1=0 if wTNB1==.
label var wTNB1 "with 351-500 as neighbors"

gen wTNB2=1 if NIL_NB23==0
replace wTNB2=0 if wTNB2==.
label var wTNB2 "with 251-500 as neighbors"

gen wTNB3=1 if NIL_NB1==0
replace wTNB3=0 if wTNB3==.
label var wTNB3 "with 250 and below as neighbors"

gen wTNB4=1 if NIL_NB2==0
replace wTNB4=0 if wTNB4==.
label var wTNB4 "with 251-350 as neighbors"
	
gen wTNB5=1 if wTNB2==1
replace wTNB5=1 if wTNB3==1
replace wTNB5=0 if wTNB5==.
label var wTNB5 "with treated district as neighbors"

gen wTNB6=1 if NIL_NB4==0
replace wTNB6=0 if wTNB6==.
label var wTNB6 "with 501-650 as neighbors"

gen wTNB7=1 if NIL_NB5==0
replace wTNB7=0 if wTNB7==.
label var wTNB7 "with 651-850 as neighbors"

gen wTNB8=1 if NIL_NB6==0
replace wTNB8=0 if wTNB8==.
label var wTNB8 "with 851 and up as neighbors"


forval i=1/8 {
	gen T`i'didl=indqual1*wTNB`i'
	gen T`i'didc=indqual2*wTNB`i'
	label var T`i'didl "labor-intensive neighbor interaction terms `i'"
	label var T`i'didc "capital-intensive neighbor interaction terms `i'"
}

saveold "$dta\itp_jde.dta", replace